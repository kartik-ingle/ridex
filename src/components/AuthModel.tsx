                ...
                <div className="w-full">
                  {step === "login" && (
                    ...
                    >
                      ...
                      <div className="mt-5 space-y-4">
                        <div className={`flex items-center gap-3 border rounded-xl px-4 py-3 border-black/20`}>
                          <Mail size={18} className="text-gray-500" />
                          <input type="email" placeholder="Email" className="w-full bg-transparent outline-none text-sm" onChange={(e) => {setEmail(e.target.value); setErr("")}} value={email}/>
                        </div>

                        <div className={`flex items-center gap-3 border rounded-xl px-4 py-3 border-black/20`}>
                          <Lock size={18} className="text-gray-500" />
                          <input type="password" placeholder="Password" className="w-full bg-transparent outline-none text-sm" onChange={(e) => {setPassword(e.target.value); setErr("")}} value={password}/>
                        </div>

                        {err && <p className="text-red-500 text-sm">*{err}</p>}

                        <button className="w-full h-11 rounded-xl bg-black text-white font-semibold hover:bg-gray-900 transition cursor-pointer flex items-center justify-center" onClick={handleLogin}>
                          {!loading? "Login": <CircleDashed size={18} color='white' className="animate-spin" />}
                        </button>
                      </div>
                      <div className="mt-6 text-center text-sm text-gray-500">
                        <p>Don't have an account?</p>
                        <div onClick={() => setStep("signup")} className="text-black font-medium hover:underline cursor-pointer">SignUp</div>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {step === "signup" && (
                    <motion.div 
                      ...
                      className="w-full"
                    >
                      <h1 className="text-xl font-semibold text-left">Create Account</h1>
                      <div className="mt-5 space-y-4">
                        ...
                        <button className="w-full h-11 rounded-xl bg-black text-white font-semibold hover:bg-gray-900 transition cursor-pointer flex items-center justify-center" disabled={loading} onClick={handleSignUp}>
                          {!loading? "Send Otp": <CircleDashed size={18} color='white' className="animate-spin" />}
                        </button>
                      </div>
                      <div className="mt-6 text-center text-sm text-gray-500">
                        <p>Already have an account?</p>
                        <div onClick={() => setStep("login")} className="text-black font-medium hover:underline cursor-pointer">Login</div>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {step == "otp" && (
                    ...
                    >
                      ...
                      <button className="mt-6 w-full h-11 rounded-xl bg-black text-white font-semibold hover:bg-gray-900 transition cursor-pointer flex items-center justify-center" onClick={handleVerifyEmail}>
